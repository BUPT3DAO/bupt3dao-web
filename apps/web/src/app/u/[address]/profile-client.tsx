...
import { cohortLabel, displayName, formatDate, hostLabel } from '@/lib/format';
...
export function ProfileClient({
  ...
  initialPosts: PageResult<PostSummary> | null;
}) {
  const { user } = useWallet();

  const [profile, setProfile] = useState<UserProfile | null>(initialProfile);
  const [posts, setPosts] = useState<PostSummary[]>(initialPosts?.items ?? []);
